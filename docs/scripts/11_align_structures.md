...
## 核心原理：表位驱动的整体对齐 (Epitope-Driven Alignment)
我们的对齐策略遵循 **“先把表位对齐，然后移动其他”** 的原则。这使得我们可以直观地比较针对同一表位的不同抗体的结合姿态。

1.  **计算变换 (Calculate Transform)**: 
    *   仅提取两个结构中 **抗原表位 (Epitope)** 部分的 Cα 原子。
    *   使用 **Kabsch 算法** 计算最佳刚体变换矩阵 ($R, t$)，使得移动结构的表位与参考结构的表位 RMSD 最小化。
2.  **应用变换 (Apply to Whole)**:
    *   将计算出的 ($R, t$) 应用于移动结构的 **完整 CIF 文件**（包含抗原剩余部分、抗体重轻链、水分子等）。
    *   公式: $v'_{all} = R \cdot v_{all} + t$
3.  **结果**: 
    *   所有输出结构的**表位部分在空间上高度重合**。
    *   抗体随之旋转和平移，保持了与抗原的相对结合关系。
    *   这允许我们在统一的坐标系下观察不同抗体对同一表位的结合角度差异。
